...
@Component({
  selector: 'app-finish-scan',
  templateUrl: './finish-scan.page.html',
  styleUrls: ['./finish-scan.page.scss'],
})
export class FinishScanPage implements OnInit {
 ...
 // check if the user (qrcode) belongs to a registered user
   ...
   // the barcode is belongs to an active user
    this.scheduleServ.finishShift(this.stringID.toString(), this.clientName.toString()).subscribe(result => {
      console.log('first' + JSON.stringify(result));
      if (result[0].error === 'Person is currently scanned OUT!') {

        // user is already scanned out
          this.start('badScan');
          this.empServ.presentAlert('Person is currently scanned OUT');
          this.stringID = '';

      } else {
        // user gets scanned OUT and records get added into the listOfScans for displaying it
